...
def trim_cloud(points, return_mask=False):
    '''
        ...
        Icput: points - cp.array of shape (N, M) where N is the number of points
        ...
        Output: if return_mask False
                    cp.array of shape (N', M) where N' <= N and the resulting array
                    is from the result of filtering points according to the algo
                else (return_mask True)
                    (cp.array described in False case, and numpy mask of length N)
    '''
    ...
    distance = cp.sqrt(cp.square(points[:, 0]) + cp.square(points[:, 1]))
    mask_r_min = r_min < distance
    mask_r_max = distance < r_max
    mask_r = cp.logical_and(mask_r_min, mask_r_max)

    # [ANGLE] select points that are within a specified angle +x-direction
    angle = cp.abs(cp.arccos(cp.divide(points[:, 0], distance)))
    ...
    distance = cp.subtract(distance, 3.1)
    slope = cp.multiply(distance, scaling)
    ground = cp.add(height, slope)

    mask_z_l = points[:, 2] > ground
    mask_z_u = points[:, 2] < max_height
    mask_z = cp.logical_and(mask_z_l, mask_z_u)

    # combine all masks to create a single mask and then select points
    mask = cp.logical_and(mask_r, mask_z)
    mask = cp.logical_and(mask, mask_angle)
...
def remove_ground(points, boxdim=0.5, height_threshold=0.01, xmin=-100, xmax=100, ymin=-100, ymax=100):
    ...
    for i in range(int((xmax - xmin)//boxdim)):
        for j in range(int((ymax - ymin)//boxdim)):
            ...
            mask_x = cp.logical_and(points[:, 0] < bxmax, bxmin < points[:, 0])
            mask_y = cp.logical_and(points[:, 1] < bymax, bymin < points[:, 1])
            mask = cp.logical_and(mask_x, mask_y)
            box = points[mask]
            ...
            if box.size != 0:
                minrow = cp.argmin(box[:, 2])
                boxLP = box[minrow].tolist()
                LPR.append(boxLP)

    if len(LPR) > 0:
        # fit lowest points to plane and use to classify ground points
        # P, C = vis.color_matrix(fns=None, pcs=[points, cp.array(LPR)])
        # vis.update_visualizer_window(None, P, colors=C)

        plane = Plane.best_fit(cp.asnumpy(LPR)) #! this could be a bottleneck
        ...
        pc_mask = cp.ones(all_points.shape[0], dtype=cp.uint8)
        plane = None
...
def plane_fit(pointcloud, planecloud=None, return_mask=False, boxdim=0.5, height_threshold=0.01):
    ...
    for i in range(int((xmax - xmin)//boxdim)):
        for j in range(int((ymax - ymin)//boxdim)):
            ...
            bxmin, bxmax = xmin + i*boxdim, xmin + (i+1)*boxdim
            bymin, bymax = ymin + j*boxdim, ymin + (j+1)*boxdim
            mask_x = cp.logical_and(planecloud[:, 0] < bxmax, bxmin < planecloud[:, 0])
            mask_y = cp.logical_and(planecloud[:, 1] < bymax, bymin < planecloud[:, 1])
            mask = cp.logical_and(mask_x, mask_y)
            box = planecloud[mask]
    ...
    if len(LPR) > 0:
        ...
        # pc_mask = cp.logical_and(D + height_threshold < pc_compare, pc_compare < D+height_max)
    else:
        pc_mask = cp.ones(pointcloud.shape[0], dtype=cp.uint8)
...
def box_range(pointcloud, return_mask=False, xmin=-100, xmax=100, ymin=-100, ymax=100, zmin=-100, zmax=100):
    '''return points that are within the boudning box specified by the optional icput parameters'''
    xrange = cp.logical_and(xmin <= pointcloud[:, 0], pointcloud[:, 0] <= xmax)
    yrange = cp.logical_and(ymin <= pointcloud[:, 1], pointcloud[:, 1] <= ymax)
    zrange = cp.logical_and(zmin <= pointcloud[:, 2], pointcloud[:, 2] <= zmax)
    mask = cp.logical_and(cp.logical_and(xrange, yrange), zrange)
...
def circle_range(pointcloud, return_mask=False, radiusmin=0, radiusmax=100):
    ...
    dists = cp.sqrt(cp.sum(pointcloud[:, :2]**2, axis=1))
    mask = cp.logical_and(radiusmin <= dists, dists <= radiusmax)
...
def random_subset(pointcloud, p):
    ...
    n_select = int(p * n)
    rand_idxs = cp.random.choice(cp.arange(n), n_select, replace=False)
...
def covered_centroid(pointcloud, centroids, radius=0.75, height=0.5, threshold=5):
    ...
    for i in range(centroids.shape[0]):
        ...
        dists = cp.sqrt(cp.sum((pointcloud[:, :2] - center[:2])**2, axis=-1))
    ...
    if len(centroids_filtered) > 0:
        centroids_filtered = cp.vstack(centroids_filtered)
    centroids_filtered = cp.array(centroids_filtered)
