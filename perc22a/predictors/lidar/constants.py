

# max radius of pointcloud to consider for prediction
INIT_PC_MAX_RADIUS = 20

# smart ground filtering parameters
SMART_GROUND_FILTER_SLICE_RADIANS = 0.1
SMART_GROUND_FILTER_SLICE_BINS = 10
SMART_GROUND_FILTER_HEIGHT_THRESHOLD = 0.13

# naive plane fit box dimensions
NAIVE_PLANE_FIT_BOX_DIM = 5
NAIVE_PLANE_FIT_HEIGHT_THRESHOLD = 0.13

# voxel size for downsampling the point cloud
DOWNSAMPLE_VOXEL_SIZE = 0.1

# cluster parameters
MAX_CLUSTER_HEIGHT_THRESHOLD = 0.5

# coloring parameters
USE_ICP_RECOLORING = False

# time
DEBUG_TIME = False

# vis debugging
VIS_PRE_CLUSTER_POINTS = False
VIS_CLUSTERS = False

# cone clustering parameters
CLUSTER_THRESHOLD_DISTANCE = 1.0
CLUSTER_HEIGHT_THRESHOLD = 2.5
CLUSTER_SIZE_THRESHOLD = 2.0
